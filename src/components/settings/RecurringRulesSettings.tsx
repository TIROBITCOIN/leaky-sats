...
function RecurringRulesModal({ onClose }: { onClose: () => void }) {
  const { categoriesById } = useLedger();
  const [rules, setRules] = useState(listRecurringRules);

  return (
    <div className="ldg-modal-backdrop" role="dialog" aria-modal="true" aria-label="반복 항목 관리" onClick={onClose}>
      <div className="ldg-modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="ldg-modal-title">반복 항목 관리</div>
...
export default function RecurringRulesSettings() {
  ...
  const count = listRecurringRules().length;
  ...
  return (
    <div className="ldg-card">
      ...
      {open && <RecurringRulesModal onClose={() => setOpen(false)} />}
    </div>
  );
}
