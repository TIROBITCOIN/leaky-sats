...
- **지갑별 독립 갱신**: 지갑 A 조회 성공 + B 실패 시, A만 갱신하고 B는 마지막 성공값 유지.
  하나라도 실패하면 홈에 "일부 지갑 조회 실패" 상태 표시 (합산값은 stale 혼합임을 명시)
- 같은 주소가 두 지갑에 중복 등록되면 outpoint 전역 중복 제거로 이중 계산 방지

`backup.ts`의 내보내기/복원 대상에 위 3개 키 추가.

### 2-6. `sync.ts` — 동기화 오케스트레이션

- 트리거: ① 홈 마운트/포그라운드 복귀(visibilitychange), ② 설정에서 "지금 동기화" 버튼,
  ③ 판매 기록 저장 직후 (전송이 이미 반영됐는지 확인용)
- 스로틀: 최소 간격 5분 (수동 버튼은 예외), 동시 실행 방지 락 (기존 `sellSaveInProgress.ts` 패턴)
...
- `partial/offline`: "지갑 연결 안 됨 — 마지막 값 표시 중" 경고 톤 (기존 시세 지연 표시 패턴 재사용)
...
- 서브 라인 한 줄만 추가: `지갑 3개 · 5분 전 동기화` (`ldg-balance-sub` 톤, 수동 모드면 미표시)
