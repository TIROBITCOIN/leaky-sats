...
반복 동기화에서는 receive/change 각각 캐시된 마지막 사용 인덱스까지 반드시 다시 조회한다. 그 경계보다 앞에서 20개 미사용 주소가 연속되더라도 조기 종료하지 않으므로, 앞쪽 주소 공백 뒤의 기존 UTXO가 합계에서 빠지지 않는다. 주소 캐시가 없으면 마지막 성공 잔고에 저장된 인덱스를 복구 경계로 사용한다.
